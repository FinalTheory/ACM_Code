...
public:
	...
	// Remove the k-th element in ascending order
	Type remove_select( int k ) {return Delete_Select(root, k);}
	// Find predecessor, return the Key itself when failed
	Type predecessor( Type Key ) {return Predecessor( root, Key );}
	// Find successor, return the Key itself when failed
	Type successor( Type Key ) {return Successor( root, Key );}
	// Get the min element in the tree
	Type get_min()
	{
		int temp = root;
	...
	// Get the max element in the tree
	Type get_max()
	{
		int temp = root;
